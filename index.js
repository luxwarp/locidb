...
class LociDB {
  ...
  */
  insert (tblName, value) {
    ...
    if (fs.existsSync(this.pathToDB + tblName + this.tblExtension)) {
      ...
      if (fs.statSync(this.pathToDB + tblName + this.tblExtension).size === 0) {
        ...
        existing = JSON.parse(fs.readFileSync(this.pathToDB + tblName + this.tblExtension, 'utf8'))
      }
    } else {
    ...
    fs.writeFileSync(this.pathToDB + tblName + this.tblExtension, JSON.stringify(existing, null, '\t'))
  }
  ...
  set (tblName, value) {
    ...
    fs.writeFileSync(this.pathToDB + tblName + this.tblExtension, JSON.stringify(data, null, '\t'))
  }
  ...
  dropRows (tblName, key, value) {
    ...
    fs.writeFileSync(this.pathToDB + tblName + this.tblExtension, JSON.stringify(rows, null, '\t'))
